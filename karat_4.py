...
def filter_rooms(instructions, treasure_room):
    present = {}
    result = []
    ...
    for instruction in instructions:
        ...
        if source in present and present[source] >= 2:
            if not (source == destination and present[source] == 2):
                if destination in treasure_room:
